    ...
    public class Mediator : IMediator
    {
                    ...
                    requestTypeKey => {
                        ...
                        if (requestInterfaceType is null)
                        {
                            throw new ArgumentException(
                                $@"{requestTypeKey
                                    .Name} does not implement {nameof(
                                    IRequest
                                )}",
                                nameof(request)
                            );
                        ...
                        var wrapperType = typeof(RequestHandlerWrapperImpl<, >)
                            .MakeGenericType(requestTypeKey, responseType);

                        return (RequestHandlerBase) (Activator
                            .CreateInstance(wrapperType)
                            ?? throw new InvalidOperationException(
        ...
        public Task Publish(
            ...
            notification switch
            {
                ...
                INotification instance => PublishNotification(
                    instance,
                    cancellationToken
                ),
                _ => throw new ArgumentException(
                    $@"{nameof(notification)} does not implement ${nameof(
                        INotification
                    )}"
                ),
            };
        ...
        )
        {
            ...
            var handler = _notificationHandlers
                .GetOrAdd(
                    notificationType,
                    t => (NotificationHandlerWrapper) (Activator
                        .CreateInstance(
                            typeof(NotificationHandlerWrapperImpl<>)
                                .MakeGenericType(t)
                        )
                        ?? throw new InvalidOperationException(
